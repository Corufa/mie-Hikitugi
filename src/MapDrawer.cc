...
void MapDrawer::DrawMapObjectsPoints(double size)                                           // MapObjectsPoints
{   // GetAllMapPoint:  GetReferenceMapPoints
    const vector<MapPoint*> &vpMPs = mpMap->GetAllMapPoints();                              // const object     & : address operator    -> : class menber acsees
    const vector<MapPoint*> &vpRefMPs = mpMap->GetReferenceMapPoints();
    ...
    for (auto* obj : objects) {
        ...
        glColor3f(static_cast<double>(c(2)) / 255,                                           // Objects points color
...
void MapDrawer::DrawMapObjects()
{
    ...
    for (auto *obj : objects) {
        ...
        if (!display_3d_bbox_) {
            ...
            // int distance_rectangle = obb(0, 0) - obb(1, 0);

            // if (distance_rectangle > 0 || distance_rectangle < 0){
                glBegin(GL_LINE_STRIP);
                glVertex3f(obb(0, 0), obb(0, 1), obb(0, 2));            // 2
                glVertex3f(obb(1, 0), obb(1, 1), obb(1, 2));            // 1
            
                // glVertex3f(obb(2, 0), obb(2, 1), obb(2, 2));            // 5
                // glVertex3f(obb(3, 0), obb(3, 1), obb(3, 2));            // 6 
            
                // glVertex3f(obb(0, 0), obb(0, 1), obb(0, 2));
                // glVertex3f(obb(4, 0), obb(4, 1), obb(4, 2));            // 4
            
                glVertex3f(obb(5, 0), obb(5, 1), obb(5, 2));            // 3
                glVertex3f(obb(4, 0), obb(4, 1), obb(4, 2));            // 4
                glVertex3f(obb(0, 0), obb(0, 1), obb(0, 2));
