...
* along with RANSACS-SLAM. If not, see <http://www.gnu.org/licenses/>.
*/
    ...
    while(1)
    {
        ...
        while (!modified_objects_.empty()) {
            ...
            for (auto* obj2 : map_objects)
            {
                ...
                if (ell.IsInside(ell2.GetCenter()) || ell2.IsInside(center) || iou > 0.15 || rel_inter > 0.2 || rel_inter2 > 0.2 || nb_common_points >= 20) { //0.4
                // if (ell.IsInside(ell2.GetCenter()) || ell2.IsInside(center) || iou > 0.3 || rel_inter > 0.4 || rel_inter2 > 0.4 || nb_common_points >= 10) { //0.4
                // if (ell.IsInside(ell2.GetCenter()) || ell2.IsInside(center) || iou > 0.05 || rel_inter > 0.10 || rel_inter2 > 0.10 || nb_common_points >= 3) {
