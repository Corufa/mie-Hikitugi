                        ...
                        const Eigen::Vector3d& center)
{
    ...
    T_center(0, 3) = center[0];                         // X            
    T_center(1, 3) = center[1];                         // Y
    T_center(2, 3) = center[2];                         // Z
...
void Ellipsoid::decompose() const {
    ...
    Eigen::Matrix4d Q_center = 0.5 * (temp + temp.transpose());                                         // Q_center

    Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> eigen_solver(Q_center.block<3, 3>(0, 0));            // Computes eigenvalues and eigenvectors of selfadjoint matrices
    Eigen::Matrix3d eig_vectors = eigen_solver.eigenvectors();                                          // 3*3 matrix PP^-1
...
BBox3 Ellipsoid::ComputeBbox() const {
    ...
    Eigen::Matrix3d M = R_ * Eigen::DiagonalMatrix<double, 3>(axes_);                           // Eigen::DiagonalMatrix<double, 3> : 3*3 diagonal matrix
    double dx = M.row(0).norm();
    double dy = M.row(1).norm();
    double dz = M.row(2).norm();
    BBox3 bb;
    bb << center_[0] - dx, center_[1] - dy, center_[2] - dz,                                     // dx
...
Eigen::Matrix<double, Eigen::Dynamic, 3> Ellipsoid::GeneratePointCloud(int sampling) const {
    ...
    pts.col(0) *= axes[0];                                              // axes of Ellipsoid
    pts.col(1) *= axes[1];
    pts.col(2) *= axes[2];                                
