...
namespace ORB_SLAM2
{
        ...
        Rw_e << std::cos(angle), -std::sin(angle), 0.0,         
                ...
                0.0, 0.0, 1.0;
        Eigen::Matrix3d transf = T_center * Rw_e;                   // projection matrix P
        C_star = transf * C_star * transf.transpose();              // ellipse by a 3*3 matrix
    ...
    void Ellipse::decompose() const {
        ...
        Eigen::Matrix3d T_c = Eigen::Matrix3d::Identity();                              // MatrixXd : X dimension
        T_c(0, 2) = -center_[0];
        T_c(1, 2) = -center_[1];
        Eigen::Matrix3d temp = T_c * C_ * T_c.transpose();                              // 3*3 matrix PP^-1
        Eigen::Matrix3d C_center = 0.5 * (temp + temp.transpose());

        Eigen::SelfAdjointEigenSolver<Eigen::Matrix2d> eigen_solver(C_center.block<2, 2>(0, 0));        // Computes eigenvalues and eigenvectors of selfadjoint matrices.
    ...
    BBox2 Ellipse::ComputeBbox() const
    {
        ...
        double xmax = std::sqrt(std::pow(axes_[0]*c, 2) + std::pow(-axes_[1]*s, 2));                    // xmax : 
