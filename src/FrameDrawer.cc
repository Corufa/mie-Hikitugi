...
cv::Mat FrameDrawer::DrawDetections(cv::Mat img)
{
    ...
    for (auto d : detections) {
        ...
        if (d.display_info) {
            ...
            // cv::putText(img, std::to_string(d.id) + "|" + ss.str() + "|" + std::to_string(d.category_id),   // ss.str() :  d:cluster
            cv::putText(img, std::to_string(d.id) + "|" + "keyboard",
...
cv::Mat FrameDrawer::DrawProjections(cv::Mat img)
{
    ...
    for (auto w : projections)
    {
        ...
        if (w.in_map) {
            ...
            // cv::Point2f pt_ellipse1,pt_ellipse2;
            // pt_ellipse1.x=c[0]-r;
            // pt_ellipse1.y=c[1]-r;
            // pt_ellipse2.x=c[1]+r;
            // pt_ellipse2.y=c[0]+r;

            // cv::ellipse(img, cv::Point2f(c[0], c[1]), cv::Size2f(axes[0], axes[1]), TO_DEG(angle), 0, 360, color, 3);
            // cv::rectangle(img,pt_ellipse1,pt_ellipse2,color);
            // cv::rectangle(img,pt1,pt2,color);
            // ellipse(img, center, axes, angle, startAngle=0, endAngle=0, color, thickness=2)
            // stringstream ss;
